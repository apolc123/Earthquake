...
# 尝试加载本地Excel数据文件（请将您的xlsx文件命名为 'earthquake_data.xlsx' 并放在代码同一目录）
DATA_FILE = 'earthquake_data.xlsx'
if os.path.exists(DATA_FILE):
    try:
        df = pd.read_excel(DATA_FILE)
        logger.info(f"从本地文件 '{DATA_FILE}' 加载数据成功")

        # 使用 load_excel 函数处理映射和验证
        # 注意: load_excel 期望 file-like object, 所以用 BytesIO
        from io import BytesIO

        with open(DATA_FILE, 'rb') as f:
            file_content = BytesIO(f.read())
            processed_df = load_excel(file_content)
            if processed_df is not None:
                earthquake_data = processed_df
                logger.info(f"数据处理成功: {len(earthquake_data)} 条记录")
            else:
                logger.warning("数据处理失败，使用原始数据")
                earthquake_data = df
    except Exception as e:
        logger.error(f"加载本地文件 '{DATA_FILE}' 失败: {e}")
        earthquake_data = pd.DataFrame()
else:
    logger.warning(f"本地数据文件 '{DATA_FILE}' 不存在，请将您的xlsx文件重命名为 '{DATA_FILE}' 并放在代码目录下")
    earthquake_data = pd.DataFrame()

# AI API配置 - 请替换为你的API密钥
# 获取免费API密钥: https://open.bigmodel.cn/
AI_API_KEY = "9c1addac66334113a21add546ba7293b.ccJQfeoWQgbNtZaH"  # 请在这里填入你的智谱AI API密钥
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="header-buttons">
                ...
                <button class="header-ai-btn" onclick="toggleAIChat()">🤖 AI助手</button>
            </div>
        </div>
            ...
            <div class="sidebar">
                ...
                <div class="card">
                    ...
                    <div class="upload-area" onclick="document.getElementById('fileInput').click()">
                        ...
                        <button class="upload-btn">替换Excel文件</button>
                    </div>
            ...
            <div class="center-column">
                ...
                <div class="timeline-panel" id="timelinePanel" style="display: none;">
                    <div class="timeline-header">
                        <div class="timeline-title">时间轴播放</div>
                    ...
                    <div class="timeline-controls-row">
                        <div class="timeline-buttons">
                            <button class="timeline-btn play" id="togglePlayBtn" onclick="togglePlayback()">播放</button>
                        </div>
        ...
        <div class="ai-chat-messages" id="aiChatMessages">
            <div class="ai-message assistant">
                ...
                <div class="ai-message-content">
                    你好!我是地震知识AI助手。你可以问我关于地震、地震灾害、防震减灾等相关问题,我会尽力为你解答。
                </div>
            </div>
        </div>
        <div class="ai-chat-input-container">
            <div class="ai-chat-input-wrapper">
                <input type="text" class="ai-chat-input" id="aiChatInput" placeholder="输入你的问题..." onkeypress="handleAIChatKeyPress(event)">
                <button class="ai-chat-send" id="aiChatSend" onclick="sendAIMessage()">发送</button>
            </div>
        </div>
    </div>
        ...
        function sendAIMessage() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    addMessage('assistant', '抱歉,我遇到了一些问题: ' + data.error);
                }
            })
            .catch(error => {
                hideTypingIndicator();
...
def call_ai_api(user_message):
    ...
    if AI_API_KEY == "your_api_key_here":
        return {
            "success": False,
            "error": "请先配置AI API密钥。访问 https://open.bigmodel.cn/ 获取免费API密钥"
        }
    ...
    try:
        ...
        system_prompt = """你是一个专业的地震知识助手。你的职责是回答用户关于地震、地震灾害、防震减灾等相关问题。
请用简洁、专业且易懂的语言回答问题。如果用户问的不是地震相关问题,请礼貌地引导他们询问地震相关内容。"""
        ...
        logger.error(f"AI API调用错误: {str(e)}", exc_info=True)
        return {"success": False, "error": f"服务暂时不可用: {str(e)}"}
...
def index():
    ...
    if not earthquake_data.empty:
        status_msg = f'<div class="success-msg">已自动加载 {len(earthquake_data)} 条地震记录</div>'
    else:
        status_msg = '<div class="error-msg">未找到数据文件，请上传Excel文件或将文件命名为 "earthquake_data.xlsx" 放在代码目录下</div>'

    # 在HTML模板中注入状态
    global HTML_TEMPLATE
    HTML_TEMPLATE = HTML_TEMPLATE.replace('<div id="uploadStatus"></div>', f'<div id="uploadStatus">{status_msg}</div>')
...
def ai_chat():
    ...
    try:
        ...
        logger.info(f"收到AI请求: {user_message}")
        ...
        if result['success']:
            ...
            logger.error(f"AI响应失败: {result.get('error', 'Unknown error')}")
...
if __name__ == '__main__':
    ...
    使用说明:
    1. 在浏览器中打开上述地址
    2. 系统将自动尝试加载代码目录下的 'earthquake_data.xlsx' 文件
    3. 如需替换数据，可上传新的Excel文件
    4. 点击右上角"🤖 AI助手"按钮打开聊天窗口
    5. 向AI助手提问地震相关知识
    6. 点击"信息检索"按钮打开检索面板
    7. 设置查询条件后点击"查询"按钮
    8. 查看地震分布地图和统计图表
    9. 点击地图右上角按钮切换散点图/热力图
    10. 点击"播放"按钮开始时间轴动画

    数据加载说明:
    - 请将您的xlsx文件重命名为 'earthquake_data.xlsx' 并放在代码同一目录下
    - 文件列要求: 地震时间/时间, 地震市区/城市, 震中位置/位置, 震级, 深度, 经度, 纬度
    - 启动日志将显示加载状态

    AI助手配置:
    - 请在代码第27行填入你的智谱AI API密钥
    - 免费获取: https://open.bigmodel.cn/
    - 注册后可获得免费额度
